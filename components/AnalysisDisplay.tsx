...
export default function AnalysisDisplay({ analysis, analyzedAt }: AnalysisDisplayProps) {
  ...
  // 提取持仓倾向关键词
  const suggestionMatch = suggestionSection.match(/(倾向于|略微倾向于|没有明显倾向|我不知道)(增持|减持)?/);
  const suggestion = suggestionMatch ? suggestionMatch[0] : '';
  
  // 判断倾向类型
  const getSuggestionStyle = () => {
    if (suggestion.includes('倾向于增持')) return { bg: 'from-green-50 to-emerald-50', border: 'border-green-200', text: 'text-green-700', icon: TrendingUp };
    if (suggestion.includes('略微倾向于增持')) return { bg: 'from-green-50 to-emerald-50', border: 'border-green-200', text: 'text-green-600', icon: TrendingUp };
    if (suggestion.includes('倾向于减持')) return { bg: 'from-red-50 to-rose-50', border: 'border-red-200', text: 'text-red-700', icon: TrendingDown };
    if (suggestion.includes('略微倾向于减持')) return { bg: 'from-red-50 to-rose-50', border: 'border-red-200', text: 'text-red-600', icon: TrendingDown };
    ...
    <div className="mt-6 space-y-4">
      ...
      {suggestion && (
        <div className={`bg-gradient-to-br ${suggestionStyle.bg} rounded-2xl p-6 border ${suggestionStyle.border}`}>
          <div>
            <h4 className={`text-lg font-bold mb-3 ${suggestionStyle.text}`}>📊 持仓倾向</h4>
            <div className="text-gray-700 leading-relaxed whitespace-pre-wrap">
              {suggestionSection}
            </div>
          </div>
      ...
      <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
        <p className="text-sm text-yellow-800 leading-relaxed">
          ⚠️ 本分析数据基于公开市场及财经信息来源，存在时效性与交易时段差异风险。持仓倾向严格依据公开事实与逻辑推导，不构成任何投资推荐或买卖指令。投资有风险，请结合自身情况与专业顾问意见审慎决策。
        </p>
      </div>
    </div>
  );
